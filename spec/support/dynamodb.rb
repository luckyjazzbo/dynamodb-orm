module DynamoDBSpecHelpers
  ...
  def define_provisioning_for(model)
    ...
    DynamodbOrm.provisioning_config[name] = min_provisioning
    model.table_indices.each do |_, index|
      DynamodbOrm.provisioning_config[name]['indices'] ||= {}
      DynamodbOrm.provisioning_config[name]['indices'][index.name] = min_provisioning
    end
  end
  ...
  def dynamodb_client
    DynamodbOrm::Connection.connect
  end
end
