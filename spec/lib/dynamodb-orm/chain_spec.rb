...
RSpec.describe DynamodbOrm::Chain do
  include_context(
    'with dynamodb table',
    'movies',
  ...
  context 'when scan mode' do
    ...
    context '#order' do
      it 'raises error' do
        expect { subject.order('desc') }.to raise_error DynamodbOrm::InvalidQuery
      end
    end
    ...
    context '#last' do
      it 'raises error' do
        expect { subject.last }.to raise_error DynamodbOrm::InvalidQuery
      end
    end
  end
  ...
  context 'when query mode' do
      ...
      context 'when there is an item' do
        ...
        context 'without where condition' do
          it 'returns last item' do
            expect {
              subject.first.title
            }.to raise_error DynamodbOrm::InvalidQuery
          end
        end
    ...
    context '#order' do
      ...
      it 'validates orders' do
        ...
        }.to raise_error DynamodbOrm::InvalidOrder
      end
    end
  end
