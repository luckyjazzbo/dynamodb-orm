...
RSpec.describe DynamodbOrm::TableUpdater do
  let(:logger) { Logger.new('/dev/null') }
  subject { described_class.new(model_class, logger: logger).update }
  class ModelWithoutFieldsAndIndicesForUpdate < DynamodbOrm::Model
  ...
  context 'when key_schema changed' do
    ...
    it 'raises exception' do
      expect { subject }.to raise_error DynamodbOrm::InvalidUpdateOperation
    end
  end

  context 'with indexes' do
    context 'when valid operations are performed' do
      class ModelWithFieldsAndIndicesForUpdate < DynamodbOrm::Model
        table primary_key: 'test_id'
    ...
    context 'when key_schema changes' do
      class ModelWithUpdatableIndex < DynamodbOrm::Model
        table primary_key: 'test_id'
        field :title, type: :string
